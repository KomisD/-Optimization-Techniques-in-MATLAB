...
for i = 1:size(starting_points, 1)
    ...
    figure(i);
    plot(k, fval);
    title(sprintf('Function value over iterations with Newton Method (Start: [%f, %f])', starting_points(i, 1), starting_points(i, 2)));
    xlabel('Iteration');
...
for i = 1:size(starting_points, 1)
    [xk, steps, points] = steepestDescentWithMinf(epsilon, starting_points(i,:), f, grad_f, second_grad_f);
    fprintf('The lowest point found (%f, %f) with %f value and %d steps \n', xk(1), xk(2), f(xk(1), xk(2)), steps);
    fval = double(f(points(:,1), points(:,2)));
    k = 1:steps+1;
...
for i = 1:size(starting_points, 1)
    [xk, steps, points] = steepestDescentWithArmigo(epsilon, starting_points(i,:), f, grad_f, second_grad_f);
    fprintf('The lowest point found (%f, %f) with %f value and %d steps \n', xk(1), xk(2), f(xk(1), xk(2)), steps);
    fval = double(f(points(:,1), points(:,2)));
    k = 1:steps+1;
...
function [xk, steps, points] = newtonMethod(epsilon, startPoint, f, gamma, grad_f, second_grad_f)
    ...
    while norm(grad_f(x(1), x(2))) > epsilon
        ...
        g = grad_f(x(1), x(2));
        step = H \ g'; % Solve H*delta = -g for delta
        x = x - gamma * step; % Update step
...
function [xk, steps, points] = steepestDescentWithMinf(epsilon, startPoint, f, grad_f, second_grad_f)
...
function [xk, steps, points] = steepestDescentWithArmigo(epsilon, startPoint, f, grad_f, second_grad_f)
    alpha = 0.01; % Line search parameter
    beta = 0.5;   % Line search parameter
